"""
Task: BALANCED Training with 500 People
SigVerify - RQ1 Core AI Performance
Creates ~10,000 pairs from 500 people
"""
...
print("=" * 70)
print("🧠 SIGVERIFY - BALANCED 500 PEOPLE")
print("=" * 70)
...
kaggle_loaded = 0
MAX_PEOPLE = 500
MAX_PER_PERSON = 3  # 3 genuine + 3 forged
...
for person_dir in genuine_folders:
    ...
    if kaggle_loaded % 100 == 0 and kaggle_loaded > 0:
...
# POSITIVE: ALL pairs of genuine (same person)
...
# NEGATIVE 1: ALL combinations genuine vs forged
for pid in person_ids:
    gen = people[pid]['genuine']
...
# NEGATIVE 2: Different person pairs (balance)
total_needed = pos_count
current_neg = len([l for l in y if l == 0])
additional = max(0, total_needed - current_neg)
