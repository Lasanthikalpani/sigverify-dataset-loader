"""
Task: Train Model - CORRECTED Person-Aware Pairs
...
print("=" * 70)
print("🧠 SIGVERIFY - CORRECTED TRAINING")
print("=" * 70)

# ============================================================
# Step 1: Load CEDAR data
# ============================================================
print("\n📂 Loading CEDAR data...")
...
for person_dir in cedar_dir.iterdir():
    ...
    if len(genuine) >= 2 and len(forged) >= 2:
        ...
        print(f"   Person {person_id}: {len(genuine)} genuine, {len(forged)} forged")

print(f"\n   ✅ Loaded {len(people)} people")

# ============================================================
# Step 2: Create CORRECT pairs
# ============================================================
print("\n🔀 Creating CORRECT pairs...")

X1, X2, y = [], [], []
person_ids = list(people.keys())

# 1. POSITIVE: Genuine vs Genuine (SAME person) = 1
...
print(f"   Positive (same person genuine): {len(X1)}")

# 2. NEGATIVE: Genuine vs Forged (SAME person) = 0
neg1_count = 0
for pid in person_ids:
    gen = people[pid]['genuine']
...
# 3. NEGATIVE: Genuine vs Genuine (DIFFERENT person) = 0
neg2_count = 0
num_diff = len([label for label in y if label == 1])
...
plt.savefig('results/training_correct.png')
plt.show()
