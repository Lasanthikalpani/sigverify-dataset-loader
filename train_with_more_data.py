"""
Train with MORE Data - Proper Solution
...
print("=" * 70)
print("🧠 SIGVERIFY - TRAIN WITH MORE DATA")
print("=" * 70)
...
# ============================================================
# Create BALANCED pairs (CORRECTED)
# ============================================================
...
# ============================================================
# FIX: Balance to the smaller count
# ============================================================
print("\n⚖️ Balancing pairs...")
...
print(f"   Before balance - Positive: {len(pos_indices)}, Negative: {len(neg_indices)}")

# Keep the smaller count
min_count = min(len(pos_indices), len(neg_indices))

# Randomly sample from both
pos_keep = np.random.choice(pos_indices, min_count, replace=False)
neg_keep = np.random.choice(neg_indices, min_count, replace=False)
keep_indices = sorted(list(pos_keep) + list(neg_keep))

X1 = [X1[i] for i in keep_indices]
X2 = [X2[i] for i in keep_indices]
y = [y[i] for i in keep_indices]

print(f"   After balance - Positive: {sum(y)}, Negative: {len(y) - sum(y)}")
...
callbacks = [
    ModelCheckpoint('models/model_more_data.keras',
...
model.save('models/model_more_data.keras')
print(f"\n✅ Model saved!")
